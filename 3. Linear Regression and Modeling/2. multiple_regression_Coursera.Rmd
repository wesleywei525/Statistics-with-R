...
```{r summary-model}
# type your code for the Exercise here, and Knit
btymodel <- glm(score~bty_avg, data = evals)
summary(btymodel)
```
...
plot(btymodel$residuals)
abline(0,0)
hist(btymodel$residuals)
```
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
m_full_elim <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level  + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_elim)$adj.r.squared
```
