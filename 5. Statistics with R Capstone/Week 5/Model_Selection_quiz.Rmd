...
```{r Q1}
q1_model <- lm(log(price) ~ log(area) + log(Lot.Area) + Bedroom.AbvGr + Overall.Qual + Land.Slope, data = ames_train)
q1_model_AIC <- stepAIC(q1_model, direction = "backward")
...
q3_model <- lm(log(price)~log(area), data = ames_train)

as.character(ames_train$Neighborhood) %>%
  data.frame(resid(q3_model),stringsAsFactors = F) %>%
  `colnames<-`(c("Neighborhood","residual")) %>%
  group_by(Neighborhood) %>%
  summarise(Mean = mean(as.numeric(residual))) %>%
  arrange(desc(Mean))
```
...
q4_model <- lm(log(price)~log(area), data = ames_train)

as.character(ames_train$Neighborhood) %>%
  data.frame(resid(q4_model)^2,stringsAsFactors = F) %>%
  `colnames<-`(c("Neighborhood","residual")) %>%
  group_by(Neighborhood) %>%
  summarise(Mean = mean(as.numeric(residual))) %>%
  arrange(desc(Mean))
...
q5_model <- lm(log(price) ~ Overall.Qual + Bsmt.Qual + Garage.Qual, data = ames_train)
summary(q5_model)
```
...
q7_model <- lm(log(price)~Overall.Cond + Overall.Qual, data = ames_train)

data.frame(ames_train$MS.SubClass,predict(q7_model),stringsAsFactors = F) %>%
  `colnames<-`(c("MS.SubClass","Predict")) %>%
  group_by(MS.SubClass) %>%
  summarise(Med = median(as.numeric(Predict))) %>%
  arrange(desc(Med))
```
...
```{r Q8}
q8_model <- lm(log(price)~Overall.Cond + Overall.Qual, data = ames_train)
hatvalues(q8_model) %>%
  sort(decreasing = T) %>%
  head()
...
Normal_sell_data <- ames_train %>%
  filter(Sale.Condition == "Normal")
q12_model <- lm(log(price)~log(area), data = Normal_sell_data)
