...
```{r Q1}
pred <- predict(model.AIC,ames_test)
exp(mean(pred))
mean(exp(pred))
```
...
data.frame(rmse.full.train,rmse.AIC.train,rmse.full.test,rmse.AIC.test)
...
```{r Q4}
n <- dim(ames_train)[1]
model.BIC <- stepAIC(model.full, k = log(n))
...
data.frame(pred.HPM.rmse, pred.BPM.rmse, pred.BPM.rmse)
```
...
# Get dataset of predictions and confidence intervals
out_q6 = as.data.frame(cbind(exp(confint(pred.test.MPM)),
                          price = ames_test$price))

# Fix names in dataset
colnames(out_q6)[1:2] <- c("lwr", "upr")  #fix names

# Get Coverage
pred.test.MPM.coverage <- out_q6 %>% summarize(cover = sum(price < lwr | price > upr)/n())
